...
/**
 * 通过事先BeanFactoryAware接口实现直接在spring中读取bean
 */
@Component
public class TestThreadPoolManager implements BeanFactoryAware {
    //用于从IOC里取对象
    private BeanFactory factory;
    //如果实现Runnable的类是通过spring的application.xml文件进行注入,
    //可通过 factory.getBean()获取，这里只是提一下
    ...
    /**
     * 用于缓存在队列中的订单，防止重复提交，在真是场景中可用redis代替，验证重复
     */
     ...
     */
    Queue<Object> msgQueue = new LinkedBlockingDeque<Object>();
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            ...
            System.out.println("系统任务太繁忙,把此订单交给(调度线程池)逐一处理，订单号："+
                    ((BusinessThread) r).getAcceptStr());
        }
    };
